    ...
    - React has been releasing a lot of new tools to help us build better UX around async work. Let's learn them with Event Hub.
    ...
    - We're using the Next.js 16.3 App Router. It uses React Server Components, so data streams through Suspense and navigation runs in transitions.
    ...
    - Open a session. The event details and comments are async Server Components that resolve independently.
    ...
    - In Chrome's Performance tab, compare two sessions with different content lengths. Reload and point out the layout shift and LCP.
    ...
    - Reload and record again in the Performance tab. The layout shift is gone because the page swaps once, but the useful event content waits for the slowest child and LCP gets worse.
    ...
    - In `EventDetails`, give `FavoriteStatus` its own Suspense boundary with a small heart skeleton so it does not hold back the details.
    - Reload. The details and form reveal first, with the comments skeleton already in the correct position. The favorite heart resolves independently, then the comments replace their skeleton. Nesting does not sequence the queries; it sequences what React is allowed to reveal.
    ...
    - If the mutation fails, the Server Action returns an error message. Await it as `const error = await toggleFavorite(eventSlug)` and call `toast.error(error)` when it returns one.
