...
- Same for the session detail page. The session info (title, speaker, schedule) is cached and prerendered. But comments and questions are dynamic — they need their own Suspense boundaries with skeletons.
...
- Since the session detail layout has `generateStaticParams`, these pages are pre-rendered at build time. The router prefetches the static shell — navigation feels instant.
...
- This is the **action props pattern**. Most devs shouldn't need to use `startTransition` themselves if they're using a transition-based router and UI components with action props. Design components like BottomNav, ChipGroup, and BackButton abstract away the async coordination. The consumer passes data and callbacks; the component handles transitions, optimistic state, and pending indicators.
...
- Questions need to update when other attendees upvote or ask new questions. We poll using `startTransition(() => router.refresh())` every 5 seconds. This refreshes the server components, fetching fresh data. The new `initialQuestions` flow down as props to the client component.
...
- Add `generateStaticParams` to the session layout. This tells Next.js to pre-render all session pages at build time — every slug is known ahead of time. Users navigate to pre-built HTML with zero loading state. Suspense skeletons only show for truly dynamic data like comments and questions.
