...
- Now let's take it further — same dropdown, switch to Offline. (Show `next.config.ts` — `experimental: { useOffline: true }`.) This is a new experimental flag. Watch — (navigate to a session.) The static shell still loads — header, tabs, skeletons all render from the cache. Now switch the throttling back to No Throttling — content streams in and fills the skeletons. The app stays usable even with no connection, and recovers gracefully when you're back online.
